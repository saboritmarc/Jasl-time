...
function showAdminPage(name){ var pm={workers:‘workers’,‘all-records’:‘all-records’,‘all-perms’:‘all-perms’,‘all-balances’:‘all-balances’,exports:‘exports’,cards:‘cards’,settings:‘settings’}; showPage(pm[name]||name); if(name===‘workers’)loadWorkers(); if(name===‘all-records’)loadAllRecords(); if(name===‘all-perms’)loadAllPerms(); if(name===‘all-balances’)loadAllBalances(); if(name===‘exports’)loadExportWorkers(); if(name===‘cards’)loadCardsWorkers(); if(name===‘settings’)loadSettings(); }
...
if(w.role===‘ADMIN’){ showPage(‘admin-home’); loadAdminStats(); loadWorkersForAdmin(); return; }
showPage(‘worker-home’); startClock(); await loadTodaySummary();
}

function doLogout(){ state.worker=null; state.todaySummary=null; clearInterval(clockInterval); document.getElementById(‘login-code’).value=’’; showPage(‘splash’); }
...
await dbPost(‘time_entries’,{worker_id:state.worker.id,entry_date:today,entry_datetime:new Date().toISOString(),entry_type:etm[action],pause_number:pauseNum,source_method:‘CODE’,is_active:true});
...
try{
var rows;
try{ var rawmh=await rpc(‘get_month_history’,{p_worker_id:parseInt(state.worker.id),p_year:y,p_month:mo}); rows=Array.isArray(rawmh)&&rawmh.length>0&&Array.isArray(rawmh[0])?rawmh[0]:rawmh; }
catch(e1){ rows=await dbGet(‘daily_records’,‘worker_id=eq.’+state.worker.id+’&work_date=gte.’+start+’&work_date=lte.’+end+’&order=work_date.asc&select=*’); }
if(!rows||!rows.length){el.innerHTML=’<div class="empty-state">Cap registre per a aquest mes</div>’;return;}
el.innerHTML=rows.map(function(r){ var bal=r.balance_minutes||0; var balCls=bal>=0?‘balance-pos’:‘balance-neg’; return ‘<div class="history-row" data-date="'+r.work_date+'" onclick="loadDayDetail(this.dataset.date)">’+’<div class="history-date">’+formatDate(r.work_date)+’</div>’+’<div class="history-info"><div class="history-hours">’+minsToHM(r.worked_minutes||0)+’ / ‘+minsToHM(r.theoretical_minutes||0)+’</div><div class="history-status">’+dayStatusTag(r.status)+(r.has_incidence?’ ⚠’:’’)+’</div></div>’+’<div class="history-balance '+balCls+'">’+(bal>=0?’+’:’’)+minsToHM(bal)+’</div></div>’; }).join(’’);
...
try{
var entries,record;
try{ var detail=await rpc(‘get_day_detail’,{p_worker_id:parseInt(state.worker.id),p_work_date:date}); entries=detail.entries||[]; record=detail.record||{}; }
catch(e1){ entries=await dbGet(‘time_entries’,‘worker_id=eq.’+state.worker.id+’&entry_date=eq.’+date+’&is_active=eq.true&order=entry_datetime.asc&select=*’); var recs=await dbGet(‘daily_records’,‘worker_id=eq.’+state.worker.id+’&work_date=eq.’+date+’&select=*’); record=recs[0]||{}; }
...
tl.innerHTML=entries.map(function(e){ var t=new Date(e.entry_datetime).toLocaleTimeString(‘ca’,{hour:‘2-digit’,minute:‘2-digit’}); var dc=e.entry_type===‘START’?‘start’:(e.entry_type===‘END’?‘end’:‘pause’); var lbl=(typeL[e.entry_type]||e.entry_type)+(e.pause_number?’ ‘+e.pause_number:’’); return ‘<div class="timeline-item"><div class="timeline-dot '+dc+'"></div><div class="timeline-time">’+t+’</div><div class="timeline-type">’+lbl+’</div></div>’; }).join(’’);
...
try{
var rows;
try{rows=await rpc(‘get_worker_day_statuses’,{p_worker_id:parseInt(state.worker.id)});}catch(e1){rows=await dbGet(‘day_status’,‘worker_id=eq.’+state.worker.id+’&is_active=eq.true&order=start_date.desc&select=*’);}
if(!rows||!rows.length){el.innerHTML=’<div class="empty-state">Cap permis registrat</div>’;return;}
el.innerHTML=rows.map(function(p){ var cc=p.compute_mode===‘THEORETICAL_HOURS’?‘tag-green’:‘tag-gray’; var cl=p.compute_mode===‘THEORETICAL_HOURS’?‘Comput teoric’:‘Zero hores’; var ch=p.comment?’<div style="color:var(--text2);font-size:12px;margin-top:4px">’+p.comment+’</div>’:’’; return ‘<div class="perm-row"><div class="card-row"><span style="font-weight:700">’+permTypeLabel(p.day_type)+’</span><span class="tag '+cc+'">’+cl+’</span></div><div class="perm-dates">’+p.start_date+’ → ‘+p.end_date+’</div>’+ch+’</div>’; }).join(’’);
...
if(type===‘new-perm’){
box.innerHTML=’<div class="modal-title">Nou permis</div><div class="input-group"><label class="input-label">Tipus</label><select id="perm-type" class="input"><option value="VACATION">Vacances</option><option value="SICK_LEAVE">Baixa medica</option><option value="PAID_LEAVE">Permis retribuit</option><option value="HOLIDAY">Festiu</option><option value="OTHER">Altres</option></select></div><div class="flex-gap"><div class="input-group" style="flex:1"><label class="input-label">Inici</label><input type="date" id="perm-start" class="input"></div><div class="input-group" style="flex:1"><label class="input-label">Fi</label><input type="date" id="perm-end" class="input"></div></div><div class="input-group"><label class="input-label">Comentari (opcional)</label><textarea id="perm-comment" class="input" placeholder="Descripcio..."></textarea></div><div class="modal-actions"><button class="btn btn-ghost" style="flex:1" onclick="closeModal()">Cancel.lar</button><button class="btn btn-primary" style="flex:1" onclick="savePerm()">Guardar</button></div>’;
...
try{
try{await rpc(‘create_day_status’,{p_worker_id:parseInt(state.worker.id),p_day_type:type,p_start_date:start,p_end_date:end,p_comment:comment});}
catch(e1){await dbPost(‘day_status’,{worker_id:state.worker.id,day_type:type,start_date:start,end_date:end,compute_mode:‘THEORETICAL_HOURS’,comment:comment,is_active:true});}
closeModal(); showToast(‘Permis creat’,‘success’); loadMyPerms();
...
var el=document.getElementById(‘balance-content’); el.innerHTML=’<div class="empty-state"><span class="loader"></span></div>’;
try{
var data;
...
catch(e1){ var now=new Date(); var yr=now.getFullYear(); var moS=String(now.getMonth()+1).padStart(2,‘0’); var rows=await dbGet(‘daily_records’,‘worker_id=eq.’+state.worker.id+’&work_date=gte.’+yr+’-01-01&select=balance_minutes,work_date’); var total=rows.reduce(function(a,r){return a+(r.balance_minutes||0);},0); var month=rows.filter(function(r){return r.work_date.startsWith(yr+’-’+moS);}).reduce(function(a,r){return a+(r.balance_minutes||0);},0); data={annual_balance:total,monthly_balance:month}; }
var ab=data.annual_balance||0; var mb=data.monthly_balance||0;
el.innerHTML=’<div style="display:grid;gap:12px;margin-bottom:24px"><div class="card"><div class="stat-lbl">Saldo mensual</div><div style="font-family:var(--mono);font-size:36px;font-weight:500;margin-top:8px;color:'+(mb>=0?'var(--accent)':'var(--warn)')+'">’+( mb>=0?’+’:’’)+minsToHM(mb)+’</div></div><div class="card"><div class="stat-lbl">Saldo anual</div><div style="font-family:var(--mono);font-size:36px;font-weight:500;margin-top:8px;color:'+(ab>=0?'var(--accent)':'var(--warn)')+'">’+( ab>=0?’+’:’’)+minsToHM(ab)+’</div></div></div>’;
...
el.innerHTML=’<div class="admin-stat"><div class="admin-stat-val">’+(data.total_workers||0)+’</div><div class="admin-stat-lbl">Treballadors</div></div><div class="admin-stat"><div class="admin-stat-val" style="color:var(--accent)">’+(data.in_progress_today||0)+’</div><div class="admin-stat-lbl">En curs avui</div></div><div class="admin-stat"><div class="admin-stat-val" style="color:var(--warn)">’+(data.incidences_today||0)+’</div><div class="admin-stat-lbl">Incidencies</div></div><div class="admin-stat"><div class="admin-stat-val" style="color:var(--accent2)">’+(data.finished_today||0)+’</div><div class="admin-stat-lbl">Finalitzats</div></div>’;
...
try{
var rows;
try{rows=await rpc(‘get_all_day_statuses’,{});}catch(e1){rows=await dbGet(‘day_status’,‘is_active=eq.true&order=start_date.desc&select=*,workers(display_name,employee_code)’);}
...
var cc2=p.compute_mode===‘THEORETICAL_HOURS’?‘tag-green’:‘tag-gray’;
var cl2=p.compute_mode===‘THEORETICAL_HOURS’?‘Teoric’:‘Zero h’;
var h2=’<div class="perm-row">’;
h2+=’<div class="card-row"><span style="font-weight:700">’+name2+’</span>’;
h2+=’<span class="tag tag-purple">’+permTypeLabel(p.day_type)+’</span></div>’;
h2+=’<div class="perm-dates">’+p.start_date+’ → ‘+p.end_date+’</div>’;
h2+=’<div style="margin-top:8px;display:flex;align-items:center;justify-content:space-between">’;
h2+=’<span class="tag '+cc2+'">’+cl2+’</span>’;
h2+=’<button class=“btn btn-ghost” style=“padding:4px 10px;font-size:11px” ‘;
h2+=‘onclick=“togglePermCompute(this.dataset.id,this.dataset.mode)” ‘;
h2+=‘data-id=”’+p.id+’” data-mode=”’+p.compute_mode+’”>’;
h2+=‘Canviar comput</button></div></div>’;
return h2;
}).join(’’);
}catch(e){el.innerHTML=’<div class="empty-state">Error</div>’;}
}
...
el.innerHTML=rows.map(function(w){ var b=w.annual_balance||0; var bc=b>=0?‘var(–accent)’:‘var(–warn)’; return ‘<div class="history-row"><div style="flex:1"><div style="font-weight:700;font-size:14px">’+(w.display_name||w.employee_code)+’</div><div style="color:var(--text2);font-size:12px">’+(w.employee_code||’’)+’</div></div><div style="font-family:var(--mono);font-size:16px;font-weight:500;color:'+bc+'">’+(b>=0?’+’:’’)+minsToHM(b)+’</div></div>’; }).join(’’);
}catch(e){el.innerHTML=’<div class="empty-state">Error</div>’;}
}
...
async function callExport(type){ showToast(‘Generant exportacio…’,‘success’); try{ if(type===‘individual’){var wId=document.getElementById(‘export-worker-select’).value;var month=parseInt(document.getElementById(‘export-month’).value);var year=parseInt(document.getElementById(‘export-year’).value);await rpc(‘generate_worker_excel_report’,{p_worker_id:wId,p_month:month,p_year:year});}else{await rpc(‘generate_all_workers_zip’,{});} showToast(‘Exportacio completada’,‘success’); }catch(e){showToast(‘Error en la generacio’,‘error’);} }
...
async function generateCards(all){ showToast(‘Generant targetes…’,‘success’); try{ if(all){await rpc(‘generate_multiple_cards_pdf’,{});}else{await rpc(‘generate_worker_card_pdf’,{p_worker_id:state.selectedCardWorker});} showToast(‘Targetes generades’,‘success’); }catch(e){showToast(‘Comprova el backend’,‘error’);} }
...
async function loginByQR(token){ try{ var worker; try{worker=await rpc(‘login_by_qr’,{p_qr_token:token});}catch(e1){var rows=await dbGet(‘workers’,‘qr_token=eq.’+encodeURIComponent(token)+’&is_active=eq.true&select=*’);worker=rows[0];} if(!worker||!worker.id)throw new Error(‘QR no valid’); state.worker=worker; afterLogin(); }catch(e){showToast(e.message||‘QR no reconegut’,‘error’);showPage(‘splash’);} }
...
function dayStatusTag(status){ var map={NOT_STARTED:’<span class="tag tag-gray">No iniciat</span>’,IN_PROGRESS:’<span class="tag tag-green">En curs</span>’,PAUSED:’<span class="tag tag-yellow">En pausa</span>’,ON_PAUSE:’<span class="tag tag-yellow">En pausa</span>’,FINISHED:’<span class="tag tag-purple">Finalitzat</span>’,COMPLETED:’<span class="tag tag-purple">Finalitzat</span>’,INCIDENCE:’<span class="tag tag-red">Incidencia</span>’,INCIDENT:’<span class="tag tag-red">Incidencia</span>’,VACATION:’<span class="tag tag-purple">Vacances</span>’,SICK_LEAVE:’<span class="tag tag-red">Baixa</span>’,PAID_LEAVE:’<span class="tag tag-yellow">Permis</span>’,HOLIDAY:’<span class="tag tag-gray">Festiu</span>’,OTHER:’<span class="tag tag-gray">Altres</span>’}; return map[status]||’<span class="tag tag-gray">’+status+’</span>’; }
function permTypeLabel(t){ var m={VACATION:‘Vacances’,SICK_LEAVE:‘Baixa medica’,PAID_LEAVE:‘Permis retribuit’,HOLIDAY:‘Festiu’,OTHER:‘Altres’}; return m[t]||t; }
