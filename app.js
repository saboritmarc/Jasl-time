...
async function doClockAction(action){
  ...
  try{
    try{ await rpc(am[action],{p_worker_id:parseInt(state.worker.id)}); }
    catch(e1){
...
async function loadMonthHistory(){
  ...
  try{
    var rows;
    try{ rows=await rpc('get_month_history',{p_worker_id:parseInt(state.worker.id),p_year:y,p_month:mo}); }
...
async function loadDayDetail(date){
  ...
  try{
    var entries,record;
    try{ var detail=await rpc('get_day_detail',{p_worker_id:parseInt(state.worker.id),p_date:date}); entries=detail.entries||[]; record=detail.record||{}; }
...
async function loadMyPerms(){
  ...
  try{
    ...
    el.innerHTML=rows.map(function(p){ var cc=p.compute_mode==='THEORETICAL_HOURS'?'tag-green':'tag-gray'; var cl=p.compute_mode==='THEORETICAL_HOURS'?'Comput teoric':'Zero hores'; var ch=p.comment?'<div style="color:var(--text2);font-size:12px;margin-top:4px">'+p.comment+'</div>':''; return '<div class="perm-row"><div class="card-row"><span style="font-weight:700">'+permTypeLabel(p.day_type)+'</span><span class="tag '+cc+'">'+cl+'</span></div><div class="perm-dates">'+p.start_date+' &rarr; '+p.end_date+'</div>'+ch+'</div>'; }).join('');
...
async function loadMyBalance(){
  ...
  try{
    var data;
    try{data=await rpc('get_worker_balance',{p_worker_id:parseInt(state.worker.id)});}
...
async function loadAdminStats(){
  ...
  try{
    var data;
    try{data=await rpc('get_admin_dashboard_summary',{});}
...
async function loadAllPerms(){
  ...
  try{
    ...
    el.innerHTML=rows.map(function(p){
      ...
      h2+='<div class="perm-dates">'+p.start_date+' &rarr; '+p.end_date+'</div>';
...
function dayStatusTag(status){ var map={NOT_STARTED:'<span class="tag tag-gray">No iniciat</span>',IN_PROGRESS:'<span class="tag tag-green">En curs</span>',PAUSED:'<span class="tag tag-yellow">Pausat</span>',FINISHED:'<span class="tag tag-purple">Finalitzat</span>',INCIDENCE:'<span class="tag tag-red">Incidencia</span>',VACATION:'<span class="tag tag-purple">Vacances</span>',SICK_LEAVE:'<span class="tag tag-red">Baixa</span>',PAID_LEAVE:'<span class="tag tag-yellow">Permis</span>',HOLIDAY:'<span class="tag tag-gray">Festiu</span>'}; return map[status]||'<span class="tag tag-gray">'+status+'</span>'; }
